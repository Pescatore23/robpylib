...
def porelabel(im, sigma = 0.35,r_max = 5):
...
#    shape = [400, 400]
    ...
    r_max = 5
    #shape = [400, 400]  
    # Load binary image, voids being true
    ######################
    inputpath = 'T:\\TOMCAT_TEST\\Fiber_bin'
    filemask = 'Fiber_bin_'
    ext= '.tif'
    os.chdir(inputpath)
    ...
    #import tif stack
    #FiberSpace = np.zeros((w,d,h),dtype=np.uint8)
    VoidSpace = np.zeros((w,d,h),dtype=np.uint8)
    ...
    for z in range(indexmax):
        ...
        VoidSpace[:,:,z]=img/255
    print('Stack Loaded') 
    VoidSpace=1-VoidSpace
    
        
    shp=VoidSpace.shape
    
    #import convex hull
    hullpath='T:\\TOMCAT_TEST\\Convex_Hull'
    hullnames='convexHull_'
    owd=os.getcwd()
    os.chdir(hullpath)
    filetype=np.uint8
